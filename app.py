...
with dashboard_tabs[2]:
    ...
    st.markdown(
        f"- `Молчание бота / dead air` — **{fnum(quality_silent)}** звонков. Это отдельный технический дефект, не сводящийся к зацикливанию.\n"
    ...
    AB_FILE = "ab_state.json"
    STATUS_OPTIONS = ["Не начат", "Запущен", "Идёт анализ", "Завершён", "Неактуален"]
    ...
    p_cont = n_human / n_vz if n_vz else 0
    p_consent = n_consent / n_cont1 if n_cont1 else 0
    p_meet = n_booked / n_consent if n_consent else 0

    st.subheader("Идеи A/B-тестов")
    st.caption("Каждый тест описан в формате: гипотеза → целевые метрики → гард-метрики → ожидаемый результат → на чём основана гипотеза. "
               "Приоритеты 1, 2, … сверху. Неактуальные тесты уезжают вниз. Статус и комментарий сохраняются между перезагрузками.")

    tests = [
        {"name": "Тест 1 · Смягчить отсечку АО / AMD (приоритет №1)",
         "hyp": "Если смягчить раннюю отсечку АО и добавить более осторожную проверку перед завершением звонка, "
                "бот перестанет терять часть живых людей, которых сейчас ошибочно принимает за автоматику.",
         "primary_metrics": "1. Доля `живой человек` среди всех соединений. 2. Конверсия в оффер среди соединений.",
         "guard_metrics": "1. Доля реальных АО, ошибочно пропущенных в разговор. 2. Средняя стоимость минуты. 3. Конверсия в встречу среди дошедших до оффера.",
         "base": f"{pct(n_human, n_vz)} живых среди соединений; робот/телефония пометили как АО только {fnum(flagged_ao)}, аналитика находит {fnum(analytic_ao)}",
         "expected": "+3 п.п. к доле живых людей среди соединений без ухудшения конверсии в встречу после оффера",
         "n": f"{fnum(n_arm(p_cont, 0.03))} на плечо (×2 = {fnum(2 * n_arm(p_cont, 0.03))})",
         "why": f"Робот/телефония отмечают АО только в {fnum(flagged_ao)} звонках, а аналитически АО/автоматики {fnum(analytic_ao)}; "
                f"на повторных звонках в автоматику уходит {repeat_ao_share:.1f}% попыток."},
        {"name": "Тест 2 · Переписать опенер под холодную базу",
         "hyp": "Если убрать ранний запрос согласия и раньше показать ценность оффера, больше клиентов останутся в разговоре и дойдут до оффера.",
         "primary_metrics": "1. Доля `прошли 1 этап` среди тех, кто дослушал первый заход. 2. Доля дошедших до оффера среди всех живых разговоров.",
         "guard_metrics": "1. Доля ранних отказов. 2. Доля `не понял бота / связь`. 3. Конверсия из оффера во встречу.",
         "base": f"{pct(n_consent, n_cont1)}",
         "expected": "+3 п.п. к прохождению 1 этапа без роста ранних отказов и переспросов",
         "n": f"{fnum(n_arm(p_consent, 0.03))} на плечо",
         "why": "На холодной базе ранние сбросы нормальны, поэтому главный резерв — поднять долю тех, кто остаётся после первых секунд и доходит до содержательной части разговора."},
        {"name": "Тест 3 · Заменить ранний сброс на переспрос / восстановление диалога",
         "hyp": "Если вместо раннего завершения бот будет один раз восстанавливать диалог после паузы, шума или неуверенного ответа, часть вовлечённых клиентов дойдёт до встречи.",
         "primary_metrics": "1. Доля `потерянных разговоров`. 2. Доля встреч среди тех, кто услышал оффер.",
         "guard_metrics": "1. Средняя длительность звонка. 2. Доля негативных реакций. 3. Доля зацикливаний.",
         "base": f"{fnum(dropped)} потерянных разговоров; {fnum(bot_drop_consent)} обрывов после согласия; {fnum(offer_bot_drop)} после оффера",
         "expected": "Снижение потерь по bot_hangup на вовлечённых клиентах без роста негатива и зацикливаний",
         "n": "по событию: bot_hangup на вовлечённых клиентах",
         "why": "В данных уже есть отдельные категории `Связь / не понял бота`, `Молчание бота`, `Бот сам завершил без явной причины` и `Потерянные разговоры` — это управляемые потери, а не клиентский отказ."},
        {"name": "Тест 4 · Оффер → встреча: дать более конкретный CTA",
         "hyp": "Если после оффера предлагать не абстрактный созвон, а готовый слот или более конкретную пользу от встречи, часть клиентов будет чаще соглашаться на следующий шаг.",
         "primary_metrics": "1. Доля назначивших встречу среди услышавших оффер. 2. Абсолютное число встреч.",
         "guard_metrics": "1. Доля отказов после оффера. 2. Доля редиректов / нецелевых. 3. Конверсия во встречу по качественным лидам.",
         "base": f"{pct(n_booked, n_consent)}",
         "expected": "+5–7 п.п. к назначению встреч среди услышавших оффер",
         "n": f"{fnum(n_arm(p_meet, 0.07))} на плечо (трафик мал → тест дольше всего)",
         "why": "После оффера конверсия уже заметно лучше, чем наверху воронки, поэтому даже умеренное улучшение CTA даст прямой прирост встреч без изменения базы."},
    ]
    ...
    if "ab_loaded" not in st.session_state:
        saved = _load_ab()
        for i in range(len(tests)):
            st.session_state[f"ab_status_{i}"] = saved.get(f"ab_status_{i}", "Не начат")
            st.session_state[f"ab_comment_{i}"] = saved.get(f"ab_comment_{i}", "")
        st.session_state["ab_loaded"] = True

    order = sorted(range(len(tests)),
                   key=lambda i: (st.session_state.get(f"ab_status_{i}") == "Неактуален", i))

    for i in order:
        t = tests[i]
        badge = st.session_state.get(f"ab_status_{i}", "Не начат")
        with st.expander(f"[{badge}] {t['name']}"):
            cc1, cc2 = st.columns([1, 2])
            cc1.selectbox("Статус", STATUS_OPTIONS, key=f"ab_status_{i}")
            cc2.text_area("Комментарий (что сделали по тесту)", key=f"ab_comment_{i}", height=70)
            st.markdown(
                f"**Гипотеза.** {t['hyp']}\n\n"
                f"**Целевые метрики.** {t['primary_metrics']}\n\n"
                f"**Гард-метрики.** {t['guard_metrics']}\n\n"
                f"**Ожидаемый результат.** {t['expected']}. Базлайн сейчас: {t['base']}.\n\n"
                f"**Нужно аудитории.** {t['n']} — при мощности 80%, доверии 95%.\n\n"
                f"**На чём основана гипотеза.** {t['why']}")

    try:
        json.dump({k: st.session_state[k] for k in list(st.session_state.keys())
                   if k.startswith("ab_status_") or k.startswith("ab_comment_")},
                  open(AB_FILE, "w", encoding="utf-8"), ensure_ascii=False)
    except Exception:
        pass
