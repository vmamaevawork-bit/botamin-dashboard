...
with dashboard_tabs[2]:
    st.subheader("Что улучшать в первую очередь")
    st.caption("Если считать ранние сбросы нормой для холодной базы, главный резерв роста встреч — не спасать всех подряд, а лучше конвертировать тех, кто уже остался в разговоре.")
    st.markdown(
        "1. **Убрать потери, где бот сам рвёт живой разговор.** Это самый ценный тип улучшения: человек уже не сбросил сразу, но встреча теряется из-за преждевременного завершения, пауз или неверной логики.\n"
        "2. **Усиливать переход после оффера.** Если клиент дослушал до оффера, это уже более тёплая часть холодной базы; здесь рост встреч обычно достигается проще, чем на старте.\n"
        "3. **Сделать реакцию на паузу и сомнение мягче.** Вместо раннего завершения боту лучше ещё раз восстановить диалог, если клиент переспросил, замолчал или ответил неуверенно.\n"
        "4. **Починить дефекты, которые ломают уже начатый разговор.** Зацикливания, пустые реплики и сломанные подстановки бьют по самым ценным звонкам, где клиент уже остался на линии.\n"
        "5. **Отдельно разбирать редиректы и нецелевые контакты.** Если бот часто выходит не на того человека, рост встреч можно получить не только скриптом, но и лучшей маршрутизацией на ЛПР."
    )
    ...
    st.caption("Каждый тест описан в формате: гипотеза → метрики → ожидаемый результат → на чём основана гипотеза. "
               "Приоритеты 1, 2, … сверху. Неактуальные тесты уезжают вниз. Статус и комментарий сохраняются между перезагрузками.")

    tests = [
        {"name": "Тест 1 · Смягчить АМД-отсечку (приоритет №1)",
         "hyp": "Бот кладёт трубку за ≤10 сек, приняв живого за автоответчик. Поднять порог AMD / "
                "включить более мягкую детекцию — вернём живых людей в разговор.",
         "metric": "Доля ветки «человек» среди дозвонов",
         "base": f"{pct(n_human, n_vz)} (сейчас рубим {fnum(amd_err)} звонков как АО)",
         "mde": "+3 п.п.",
         "n": f"{fnum(n_arm(p_cont, 0.03))} на плечо (×2 = {fnum(2 * n_arm(p_cont, 0.03))})",
         "why": "36% дозвонов бот рубит ≤10 сек; те же номера на других попытках выходят на живых — "
                "это явные ложные срабатывания AMD. Самый дешёвый верх воронки."},
        {"name": "Тест 2 · Переписать опенер (узкое место воронки)",
         "hyp": "Опенер просит согласие («Тридцать секунд, ладно?») раньше ценности. Дать крючок/выгоду "
                "до запроса согласия — поднимем долю согласившихся слушать.",
         "metric": "Доля давших согласие среди продолживших разговор",
         "base": f"{pct(n_consent, n_cont1)}",
         "mde": "+3 п.п.",
         "n": f"{fnum(n_arm(p_consent, 0.03))} на плечо",
         "why": "Здесь теряем ~92% — крупнейшая утечка по объёму, максимальный потолок роста."},
        {"name": "Тест 3 · Не дать боту бросать вовлечённых",
         "hyp": "Бот завершает звонок при паузе/«связь подвисает», теряя готовых "
                f"({fnum(dropped)} «готовых», {fnum(bot_drop_consent)} после согласия, "
                f"{fnum(offer_bot_drop)} после оффера). Заменить сброс на переспрос.",
         "metric": "Доля bot_hangup среди вовлечённых; доля дошедших до встречи",
         "base": "около половины неназначений встречи — по нашей вине",
         "mde": "фикс-логика (мерить как долю bot_hangup на вовлечённых)",
         "n": "по событию: bot_hangup на вовлечённых клиентах",
         "why": "Причины неназначения: «бот прервал» 64 + «непонимание/связь» 41 ≫ реальный отказ ~12."},
        {"name": "Тест 4 · Оффер → встреча: усилить призыв",
         "hyp": "После оффера тестировать более конкретный CTA (готовый слот / выгода эксперта).",
         "metric": "Доля назначивших встречу среди услышавших оффер",
         "base": f"{pct(n_booked, n_consent)}",
         "mde": "+7 п.п.",
         "n": f"{fnum(n_arm(p_meet, 0.07))} на плечо (трафик мал → тест дольше всего)",
         "why": "Конверсия уже неплохая (~37%), но объём мал → приоритет ниже."},
    ]
    ...
    for i in order:
        ...
        with st.expander(f"[{badge}] {t['name']}"):
            ...
            st.markdown(
                f"**Гипотеза.** {t['hyp']}\n\n"
                f"**Метрики.** {t['metric']}. Базлайн сейчас: {t['base']}.\n\n"
                f"**Ожидаемый результат.** {t['mde'].rstrip('.')}. Нужно аудитории: {t['n']} — "
                f"при мощности 80%, доверии 95%.\n\n"
