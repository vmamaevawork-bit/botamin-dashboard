    ...
    ab_tests = pd.DataFrame([
        {
            ...
            "Гипотеза": "Если в начале разговора сначала дать конкретную выгоду одной фразой, а "
                        "обязательный запрос согласия оставить сразу после неё (не убирая), то "
                        "«Прошли 1 этап ÷ Дослушали начало» вырастет (ориентир +2–3 п.п.), потому что "
                        "клиент слышит выгоду до решения продолжать, а юридическое согласие сохраняется.",
            "Метрики": bullets([
                "Прошли 1 этап ÷ Дослушали начало",
                "Дошли до оффера ÷ Живые разговоры",
            ]),
            "Гард-метрики": bullets([
                "Согласие зафиксировано ÷ Разговоры",
        ...
        },
        {
            "Проблема": (f"Клиенты отваливаются в начале — из {fnum(n_cont1)} дослушавших начало "
                         f"до оффера доходят {fnum(n_consent)}, теряем {fnum(lost_offers)} офферов "
                         f"({pct(lost_offers, n_cont1)})"),
            "Гипотеза": "Если объединить обязательный запрос согласия с ценностью в одной фразе "
                        "(«Звонит робот [компании], можем за минуту рассказать, как [выгода]?»), то "
                        "«Прошли 1 этап ÷ Дослушали начало» вырастет, потому что запрос согласия "
                        "перестаёт звучать как пустой формальный барьер.",
            "Метрики": bullets([
                "Прошли 1 этап ÷ Дослушали начало",
                "Дошли до оффера ÷ Живые разговоры",
            ]),
