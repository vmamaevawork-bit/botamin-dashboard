...
[data-testid="stTable"] table:has(thead th:nth-child(4)):not(:has(thead th:nth-child(5))) td:nth-child(1) {width:24%;}
[data-testid="stTable"] table:has(thead th:nth-child(4)):not(:has(thead th:nth-child(5))) th:nth-child(2),
[data-testid="stTable"] table:has(thead th:nth-child(4)):not(:has(thead th:nth-child(5))) td:nth-child(2) {width:34%;}
[data-testid="stTable"] table:has(thead th:nth-child(4)):not(:has(thead th:nth-child(5))) th:nth-child(3),
[data-testid="stTable"] table:has(thead th:nth-child(4)):not(:has(thead th:nth-child(5))) td:nth-child(3) {width:21%;}
[data-testid="stTable"] table:has(thead th:nth-child(4)):not(:has(thead th:nth-child(5))) th:nth-child(4),
[data-testid="stTable"] table:has(thead th:nth-child(4)):not(:has(thead th:nth-child(5))) td:nth-child(4) {width:21%;}
</style>
    ...
    ab_tests = pd.DataFrame([
        {
            ...
            "Гипотеза": "Если убрать ранний запрос согласия из начала разговора, то «Прошли 1 этап ÷ "
                        "Дослушали начало» вырастет (ориентир +2–3 п.п.), потому что клиент раньше слышит "
                        "ценность и не отсекается формальным вопросом на старте.",
            "Метрики": bullets([
                "Прошли 1 этап ÷ Дослушали начало",
                "Дошли до оффера ÷ Живые разговоры",
            ]),
            "Гард-метрики": bullets([
                "Ранние отказы ÷ Живые разговоры",
                "«Не понял / связь» ÷ Живые разговоры",
                "Назначили встречу ÷ Дошли до оффера",
            ]),
        },
        {
            ...
            "Гипотеза": "Если при первой паузе или шуме бот один раз переспросит вместо завершения звонка, "
                        "то «Потерянные разговоры ÷ Вовлечённые» снизится, потому что часть обрывов "
                        "сейчас — техническая пауза или шум, а не отказ клиента.",
            "Метрики": bullets([
                "Потерянные разговоры ÷ Вовлечённые",
                "Назначили встречу ÷ Услышали оффер",
            ]),
            "Гард-метрики": bullets([
        ...
        },
        {
            ...
            "Гипотеза": "Если после оффера предлагать готовое время на выбор вместо общего «давайте "
                        "созвонимся», то «Назначили встречу ÷ Услышали оффер» вырастет (ориентир +3–5 п.п.), "
                        "потому что конкретный выбор снижает усилие клиента и подталкивает к решению.",
            "Метрики": bullets([
                "Назначили встречу ÷ Услышали оффер",
                "Число встреч (абсолютное)",
            ]),
            "Гард-метрики": bullets([
                "Отказы после оффера ÷ Услышали оффер",
                "Нецелевые / редиректы ÷ Звонки",
            ]),
        },
        {
            "Проблема": (f"Бот отсекает часть живых как автоответчик в первые секунды — "
                         f"{fnum(amd_err)} звонков бот завершил за ≤10 сек"),
            "Гипотеза": "Если поднять порог ранней отсечки АО (AMD) и проверять ещё раз перед сбросом, "
                        "то «Человек ÷ Дозвонились» вырастет, потому что часть живых сейчас рубится "
                        "в первые секунды как автоответчик.",
            "Метрики": bullets([
                "Человек ÷ Дозвонились",
                "Дошли до оффера ÷ Дозвонились",
            ]),
            "Гард-метрики": bullets([
                "Реальные АО, пропущенные в разговор ÷ Дозвонились",
                "Средняя стоимость минуты",
            ]),
        },
        {
            "Проблема": "Доля живых ответов и назначенных встреч заметно зависит от часа и дня недели",
            "Гипотеза": "Если сдвинуть обзвон в часы и дни с наибольшей долей живых ответов, то "
                        "«Человек ÷ Дозвонились» вырастет, потому что в эти окна выше шанс попасть "
                        "на человека, а не на автоматику.",
            "Метрики": bullets([
                "Человек ÷ Дозвонились (по окнам)",
                "Назначили встречу ÷ Дозвонились",
            ]),
            "Гард-метрики": bullets([
                "Доля АО по окнам",
                "Средняя стоимость минуты",
